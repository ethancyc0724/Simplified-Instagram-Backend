...
class FollowsRepo:
    ...
    async def create_request(self, db: AsyncSession, follower_id: str, following_id: str) -> Follow:
    ...
    async def list_follows(
        ...
        if list_type == "following":
            base = select(User).join(Follow, Follow.following_id == User.user_id).where(
                Follow.follower_id == current_user_id, Follow.status == status
            )
        else:
            base = select(User).join(Follow, Follow.follower_id == User.user_id).where(
