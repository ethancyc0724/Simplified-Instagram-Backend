...
class FollowsService:
    ...
    async def list_follows(self, db: AsyncSession, current, list_type: str, status: str, page: int, limit: int):
        ...
        cache_key = k("follows", current["user_id"], list_type, status, str(page), str(limit))
        cached = await get_json(cache_key)
        if cached:
            return cached, "HIT"

        users, total_pages = await self.repo.list_follows(
    ...
    async def request_follow(self, db: AsyncSession, current, user_id: str):
        ...
        if exist:
            raise HTTPException(status_code=400, detail="You have already sent a follow request.")

        follow = await self.repo.create_request(db, str(follower_id), str(following_id))
        ...
        await self.events.create_event(
            ...
            type="friend_request",
            message=f"{(follower_user.name or follower_user.username)}向你發出追蹤請求",
            metadata={
                "follower": {
        ...
        return {"data": {"follows_id": str(follow.follows_id)}, "message": "ok"}
    ...
    async def act_on_follow(self, db: AsyncSession, current, follows_id: str, body) -> Dict[str, Any]:
        ...
        if body.status == "agree":
            if follow.status == "agree":
                await delete_pattern(k("follows", str(follow.follower_id), "*"))
                await delete_pattern(k("follows", str(follow.following_id), "*"))
                await delete(k("events", str(follow.follower_id), "1", "10"))
                return {"data": {"follows_id": follows_id}, "message": "ok"}
            ...
            await self.events.create_event(
                ...
                type="friend_agree",
                message=f"{following_user.username}同意你的追蹤請求",
                metadata={"following": {"user_id": str(following_user.user_id), "username": following_user.username,
